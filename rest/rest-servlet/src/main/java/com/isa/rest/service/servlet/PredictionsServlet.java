...
public class PredictionsServlet extends HttpServlet {
  ...
  public void init() {
    ...
    this.predictions.setServletContext(this.getServletContext());
  }
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response) {
    ...
    // Otherwise, return the specified Prediction.
      ...
      Prediction prediction = predictions.getPrediction(key);
  ...
  public void doPut(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      /*
       * To simplify the hack, assume that the PUT request has exactly two parameters: the id and either who or what. Assume, further, that the id comes first. From the
       * client side, a hash character # separates the id and the who/what, e.g.,
       *
    ...
    Prediction p = predictions.getPrediction(Integer.valueOf(key.trim()));
    if (p == null) { // not found?
      String msg = key + " does not map to a Prediction.\n";
      sendResponse(response, predictions.toXML(msg), false);
    } else { // found
